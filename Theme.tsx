...
// Get the globally exposed nonce value
const nonce = (window as any).__SUPERSER_CSP_NONCE__ || '';
console.log("nonce value [superset-frontend/packages/superset-core/src/ui/theme/Theme.tsx]: ", nonce);
...
export class Theme {
  ...
  setConfig(config: AnyThemeConfig): void {
    ...
    this.updateProviders(
      this.theme,
      this.antdConfig,
      createCache({ key: 'superset', nonce: nonce }),
    );
  }
  ...
  SupersetThemeProvider({ children }: { children: React.ReactNode }) {
    ...
    const [themeState, setThemeState] = React.useState({
      ...
      emotionCache: createCache({ key: 'superset', nonce:nonce }),
    });
